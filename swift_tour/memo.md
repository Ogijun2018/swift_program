...
`:`よりも前のクラスを「小クラス」、後ろのクラスを「親クラス」と呼ぶ。

小クラスには親クラスのプロパティやメソッドが引き継がれる。

継承は「コードが重複してしまうケース」を解決するために用意されている機能。

構造体とクラスの大きな違いは「**値型と参照型の違い**」とあったが、「**クラスには継承という機能がある**」ことも大きな違いの一つ。

継承の書式

```
class クラス名:親クラス名, 批准させるプロトコル名 {
}
```

親クラスを指定せずにクラスを宣言すると、それらのクラスは親クラスのないクラスとして扱われる。

クラスには宣言や定義されていないプロパティやメソッドが登場してくるが、そのような場合は**それらは親クラスから継承されているのだな**と考える。

クラスには 1 つの親クラスしか指定できないが、複数のプロトコルを批准できる。

複数のプロトコルを批准する場合は`,`で区切って並べていく。

```
class Japan:Asia, KyotoProtocol, WashingtonConvention {
}
```

## タイプキャスティング

辞書 info からオリジナルの画像を取り出すコード

```
let uiImage = info[.originalImage] as! UIImage
```

辞書 info にはユーザーが撮影した画像や、撮影時刻・撮影場所などのメタデータなど、異なる型のデータが格納されている。

これらの型を一挙にやり取りするために、辞書の値の型に Any 型が指定されている。

Any 型のメリット・デメリット

メリット：どのような値でも格納できるので、1 つの引数であらゆる型のデータに辞書で対応することができる（そうでないとかなり冗長的なコードになってしまう）

デメリット：処理の実行前に、プログラムの正当性をチェックするという型の存在目的がなくなってしまう

Any 型が使われているものに対しては、**何よりも先に、そのデータに適した型に変換することが大切**。

この変換を行うことをタイプキャスティングという。
...
- どちらの方向にキャスティングしたとしても、変数内のインスタンス自体は何も変わらない

- タイプキャスティングは継承関係ないでしか実行できない。継承関係にないクラス間でタイプキャスティングするとエラーになる

- アップキャスティングは常に成功する（継承の関係を考えると当然）

ダウンキャスティングの失敗によるアプリのクラッシュを未然に防ぐためのキーワードが`as!`
...
デリゲート（代理人）とは、**あるクラスだけでは処理できない命令を、そのクラスの代わりに行うクラス**のこと。
...
let taro = Defender()    // taro君は被告人
taro.delegate = Lawyer() // taro君は代理人として弁護士を呼ぶ
taro.delegate!.defend()  // taro君に代わって代理人が「意義あり!」と表示する
```

デリゲートは必ずプロトコルを批准する。

代理人が本物であるかどうかをチェックしなければいけない。

現実世界では弁護士資格の有無で確認できるが、このコードには弁護士資格にあたるコードが存在しない。
...
class Defender {
  // delegateプロパティはLawyerLicenseを批准したクラスのインスタンスしか格納できなくする
  var delegate:LawyerLicense?
}

// Lawyerが弁護士資格を持っていることを示すために、LawyerクラスにLawyerLicenseプロトコルを指定する
...
親である ContentView 構造体は、画面上に写真を表示するために image プロパティを持っている。

しかし、その画像は子である ImagePicker 構造体によって取得される。

そのため、親である ContentView 構造体はどうにかして子から画像を受け取る必要がある。

しかし、構造体は値型なので、引数で渡しても元々の値が変更されないので親の image プロパティは変更されない。

一方、@State と@Binding を使うと、値型から参照型になるので子の image プロパティを変更すると親の image プロパティも変更される。（逆も同じ）

@State のついたプロパティの値を子の UI 部品に渡す場合は、引数の前に`$`をつける。
